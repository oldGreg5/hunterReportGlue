...
function writeToNicEdit(text) {
    let textFromNic = new nicEditors.findEditor('combinedText');
    textFromNic.setContent(text);
    console.log(textFromNic.getContent());
}
...
async function loadAndParseDescriptionFile() {
    ...
    new nicEditor({
        ...
        maxHeight: window.innerHeight - 320 // Further reduced height to avoid scrolling
    }).panelInstance('combinedText');

    // Add resize listener to adjust editor height when window resizes
    window.addEventListener('resize', function() {
        // Find the nicEditor instance
        let nicInstance = nicEditors.findEditor('combinedText');
        if (nicInstance) {
            // Get the editor element
            let editorElem = nicInstance.elm.parentNode.querySelector('.nicEdit-main');
            if (editorElem) {
                // Set the height dynamically
                editorElem.style.height = (window.innerHeight - 320) + 'px';
    ...
    // Manually adjust editor height after initialization
    setTimeout(function() {
        let editorElem = document.querySelector('.nicEdit-main');
        if (editorElem) {
            editorElem.style.height = (window.innerHeight - 320) + 'px';
            editorElem.style.maxHeight = 'none';
            console.log('Adjusted editor height to:', (window.innerHeight - 320) + 'px');
        }
    }, 500);
}
